...
class RunningWorkout(BasicWorkout):
    def __init__(self):
        super().__init__()
    ...
    def get_core(self, warming_up, finale):
        ...
        e4 = df[df['name'] == 'Dead Bug - Static'].sample(n=1)
        e5 = df[df['name'] == 'Shoulder Taps'].sample(n=1)
        e6 = df[df['name'] == 'Plank'].sample(n=1)

        e7 = df[df['name'] == 'Bosu Mountain Climbers'].sample(n=1)
        e8 = df[df['name'] == 'Bosu Clean and Press'].sample(n=1)
        e9 = df[df['name'] == 'Flutter Kicks 4x'].sample(n=1)
